...
function ProductList({ onHomeClick }) {
    ...
    const calculateTotalQuantity = () => {
        return itemsInCart ? itemsInCart.reduce((total, item) => total + item.quantity, 0) : 0;
    };

    const [totalQuantity, setTotalQuantity] = useState(calculateTotalQuantity());
    
    // set up an effect to recalculate the total quantity whenever itemsInCart changes and
    // update the addedToCart state
    useEffect(() => {
        // Update the total quantity whenever itemsInCart changes
        setTotalQuantity(calculateTotalQuantity());

        // Update the addedToCart state based on itemsInCart
        const newAddedToCart = {};
        itemsInCart.forEach(item => {
            newAddedToCart[item.name] = true; // Mark the item as added to cart
        });
            ...
            {!showCart ? (
                <div className="product-grid">
                    {plantsArray.map((category, index) => (
                        <div key={index}>
                            <h1 className="plantname_heading"><div><hr />{category.category}<hr /></div></h1>
                                    ...
                                    <div className="product-card" key={plantIndex}>
                                        ...
                                        {addedToCart[plant.name] ? (
                                            <button
                                                ...
                                                disabled="true" // Disable the button if the product is already added to the cart
